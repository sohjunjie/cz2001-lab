...
public class Graph {
	...
	private LinkedHashMap<Pair,Integer> Distance;       // matrixDistance to keep track of shortest path distance between vertices. 
	                                       //MatrixDistance[i][j] = shortest path distance between vertices i and j
	private LinkedHashMap<Pair,Integer> Predecessor;    // MatrixPredecessor keeps track of predecessor vertex along shortest path.
	...
	public Graph(int vertices){ //Class constructor
		...
		this.Distance = (LinkedHashMap<Pair,Integer>) new LinkedHashMap();  //Initialize matrixDistance
		this.Predecessor = (LinkedHashMap<Pair,Integer>) new LinkedHashMap();;  //Initialize matrixPredecessor
	...
	public void generateRandomGraph(int edges){  //Method to generate random graph given number of edges required
		...
		while(this.getE() < edges){ // while number of edges of graph is less than required
			...
			System.out.println("startVertexNum : " + startVertexNum + ", endVertexNum : " + endVertexNum);
		}
	...
	private void runBFS(int startVertexNum){ //Method to run BFS given id of startVertex
		...
		Pair pair;                                // pair reference object
		...
		pair = new Pair(startVertex.getId(),startVertex.getId());
		
		Distance.put(pair,0); //set distance of startVertex from itself = 0
		...
		while(!queue.isEmpty()){  // while queue is not empty
			...
			for(Vertex iteratorVertex : adjacencyListOfVertex){ // Iterate over vertices in the retrieved adjacencyList
				if(!iteratorVertex.isMarked()){      // if iteratorVertex is unmarked
					...
					predecessorDistance = Distance.get(pair);
					pair = new Pair(startVertex.getId(),iteratorVertex.getId());
					//Distance.[startVertex.getId()][iteratorVertex.getId()] = matrixDistance[startVertex.getId()][vertex.getId()] + 1;
					Distance.put(pair, predecessorDistance + 1);
					Predecessor.put(pair, vertex.getId());
	...
	public void allPairsShortestPath(){ //Method to calculate all pairs shortest path in graph
		System.out.println("Entered allPairsShortestPath");
		for(int i = 0;i < this.V;i++){   //Run the BFS algorithm on all vertices of graph
			System.out.println("Vertex : " + i);
			this.runBFS(i);
		}
		System.out.println("Exiting allPairsShortestPath");
	}
	
	public void printAllPairsShortestPath(){
		//Iterator<Pair,Integer> it = Distance.entrySet().iterator();
		for(Pair iteratorPair : Distance.keySet()){
			System.out.println(iteratorPair.toString() + " - " + Distance.get(iteratorPair));
		}
	}
	
	public void printBFSPath(int startVertexNum, int endVertexNum){
